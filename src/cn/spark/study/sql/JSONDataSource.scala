...
object JSONDataSource {
  def main(args: Array[String]): Unit = {
    ...
    // 创建学生成绩DataFrame
    val studentScoresDf = sqlContext.read.json("hdfs://hadoop-3:9000/spark-study/students.json")
    
    // 查询出分数大于80分的学生成绩信息，以及学生姓名
    studentScoresDf.registerTempTable("student_scores")
    val goodStudentScoresInfo = sqlContext.sql("select name,score from student_scores where score>=80")
    
    val goodStudentNames = goodStudentScoresInfo.rdd.map(row ⇒ (row(0))).collect()
    
    // 创建学生基本信息DataFrame
    val studentInfoJSONs = Array("{\"name\":\"Leo\", \"age\":18}", 
        "{\"name\":\"Marry\", \"age\":17}",
        "{\"name\":\"Jack\", \"age\":19}")
    val studentInfoJSONsRDD = sc.parallelize(studentInfoJSONs, 1)
    ...
    val goodStudentsRDD = 
      goodStudentScoresInfo.rdd.map(row ⇒ (row.getAs[String]("name"), row.getAs[Long]("score")))
    ...
    goodStudentsDf.write.format("json").save("hdfs://hadoop-3:9000/spark-study/good-students-scala")        
